...
DLL_EXPORT typedef struct SherpaNcnnDecoderConfig {
...
DLL_EXPORT typedef struct SherpaNcnnResult {
  const char *text;
  // TODO: Add more fields
} SherpaNcnnResult;

DLL_EXPORT typedef struct SherpaNcnnRecognizer SherpaNcnnRecognizer;
...
DLL_EXPORT void DestroyRecognizer(SherpaNcnnRecognizer *p);
...
DLL_EXPORT void AcceptWaveform(SherpaNcnnRecognizer *p, float sample_rate,
...
DLL_EXPORT SherpaNcnnResult *GetResult(SherpaNcnnRecognizer *p);

/// Destroy the pointer returned by GetResult().
///
/// @param r A pointer returned by GetResult()
DLL_EXPORT void DestroyResult(const SherpaNcnnResult *r);

/// Reset the recognizer, which clears the neural network model state
/// and the state for decoding.
///
/// @param p A pointer returned by CreateRecognizer().
DLL_EXPORT void Reset(SherpaNcnnRecognizer *p);

/// Signal that no more audio samples would be available.
/// After this call, you cannot call AcceptWaveform() any more.
///
/// @param p A pointer returned by CreateRecognizer()
DLL_EXPORT void InputFinished(SherpaNcnnRecognizer *p);
...
DLL_EXPORT int32_t IsEndpoint(SherpaNcnnRecognizer *p);
